...
$content = $content.Replace($subBlock, [string]::Join("", $newSubParts))

# ============================================================
# 4) WHERE outer-join line
# ============================================================
...
$content = $content.Replace($oldJoin, [string]::Join($CRLF, $joinParts))
...
# ============================================================
# 6) detail-band computes: clalu -> saldo_awal; cmutasi -> 12x cbln{N}; cini -> cttl  (detail-level uses "cini" not "compute_4")
# ============================================================
...
$cini = Get-ComputeObj $content "detail" "cini"
$ciniTail = $cini.Full.Substring($cini.Full.IndexOf('name=cini visible="1" ') + 'name=cini visible="1" '.Length)
$ttlX = $baseX + 12*$colStep
$ttlExpr = "saldo_awal + cbln1"
for ($n=2; $n -le 12; $n++) { $ttlExpr += " + if(arg_jml_bulan>=${n},cbln${n},0)" }
$newCttl = "compute(band=detail alignment=`"1`" expression=`"$ttlExpr`"border=`"0`" color=`"33554432`" x=`"$ttlX`" y=`"4`" height=`"76`" width=`"622`" format=`"#,##0.00`" html.valueishtml=`"0`"  name=cttl visible=`"1`" $ciniTail"
$content = $content.Replace($cini.Full, $newCttl)

# ============================================================
# 7) trailer.1 (group 1 "fincatcode" subtotal) - identical mechanism/net-split-detection to rekap script
# ============================================================
...
# ============================================================
# 8) trailer.2 (group 2 "parentcode" subtotal) - REAL rendering band at detail level (unlike rekap's suppressed header.2)
#    Always plain clalu/cmutasi based (never net-split, confirmed even for pendapatan/pajak) -> FULL 12-column replication
# ============================================================
$c6b = Get-ComputeObj $content "trailer.2" "compute_6"
$content = $content.Replace($c6b.Full, $c6b.Full.Replace("sum(clalu for group 2)", "sum(saldo_awal for group 2)"))

$c8b = Get-ComputeObj $content "trailer.2" "compute_8"
$c8bTail = $c8b.Full.Substring($c8b.Full.IndexOf('name=compute_8 visible="1" ') + 'name=compute_8 visible="1" '.Length)
$t2CblnParts = @()
for ($n=1; $n -le 12; $n++) {
    $x = $baseX + ($n-1)*$colStep
    $expr = "sum(cbln${n} for group 2)"
    $t2CblnParts += "compute(band=trailer.2 alignment=`"1`" expression=`"$expr`"border=`"0`" color=`"33554432`" x=`"$x`" y=`"8`" height=`"76`" width=`"$colWidth`" format=`"#,##0.00`" html.valueishtml=`"0`"  name=t2_cbln${n} visible=`"arg_jml_bulan>=${n}`" $c8bTail"
}
$content = $content.Replace($c8b.Full, [string]::Join($CRLF, $t2CblnParts))

$c7b = Get-ComputeObj $content "trailer.2" "compute_7"
$t2TtlExpr = "sum(saldo_awal for group 2) + sum(cbln1 for group 2)"
for ($n=2; $n -le 12; $n++) { $t2TtlExpr += " + if(arg_jml_bulan>=${n},sum(cbln${n} for group 2),0)" }
$newC7b = $c7b.Full -replace [regex]::Escape('expression="sum(clalu + cmutasi for group 2)"'), ('expression="' + $t2TtlExpr + '"')
Req ($newC7b -ne $c7b.Full) "compute_7 (trailer.2) expression substitution had no effect"
$newC7b = $newC7b.Replace("name=compute_7 visible=", "name=t2_cttl visible=")
$content = $content.Replace($c7b.Full, $newC7b)

# ============================================================
# write output (preserve UTF-16LE + BOM)
# ============================================================
[System.IO.File]::WriteAllText($DestFile, $content, [System.Text.Encoding]::Unicode)
Write-Output "OK: wrote $DestFile (baseX=$baseX, netSplit=$hasNetSplit)"
