...
"@ "TSALES2 88 sudah ter-UPDATE ke hpp_new? (16=sudah, 0=belum)"

Tab @"
select count(*) n_ok
  from ZZ_WIPFIX_MAP_R2_20260810 m, gl_journal g
 where g.voucher=m.voucher and g.account_id='102-020' and g.modul_id='AS'
   and abs(g.debet - round(m.hpp_new,2)) < 0.01
"@ "GL_JOURNAL Dr 102-020 sudah ter-UPDATE? (16=sudah, 0=belum)"
$c.Close()
