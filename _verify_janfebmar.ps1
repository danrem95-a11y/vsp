...
Tab "select refresh_id,periode,ts_start,ts_end,status from refresh_ledger where periode in ('2026-01','2026-02','2026-03') order by ts_start desc" "0. refresh_ledger Jan/Feb/Mar terbaru"
...
# WIP per-voucher independen utk Jan, Feb, Mar
Tab @"
select gr.persediaan account_id, count(*) n_voucher_mismatch, cast(sum(abs(v.mutasi-v.glval)) as numeric(20,2)) total_selisih
  from (
    select gr2.persediaan persediaan, s1.bukti_id,
     ...
     where s1.bukti_id=s2.bukti_id and s1.tipe_trans='88' and s1.tgl between '2026-01-01' and '2026-03-31'
...
"@ "3. WIP per-voucher independen (Jan-Mar): harus 0 baris"

Tab "select gate_id,periode_from,sumber,n_pelanggaran,status,tgl_cek from costing_gate_result order by tgl_cek desc" "4. costing_gate_result (cek apakah sudah terisi utk run terbaru)"
Tab "select result_id,tgl_check,periode_from,account_id,status from wip_out_gate_result order by tgl_check desc" "5. wip_out_gate_result (cek apakah sudah terisi utk run terbaru)"
$c.Close()
