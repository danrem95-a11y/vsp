...
--ISNULL(STOK_RP.BELI,0)AS BELI_RP,
  	...
  	FROM IM_PRODUK,
			(  SELECT 	(SINV.STOK_ID) AS KEY1,   
				...
				FROM     SINV 
			WHERE    	( MONTH(SINV.PERIODE) = MONTH('2026-04-01')) AND
							( YEAR(SINV.PERIODE) = YEAR('2026-04-01')) 
...
((a.group_product = '') or 1 = 1) and
((a.awal <>0 or a.awal_rp <>0 or a.mutasi_in <> 0 or a.beli <> 0 or a.beli_rp <>0 or
a.ret_beli<>0 or a.ret_beli_rp <> 0 or a.ekspedisi <> 0 or a.ekspedisi_rp <> 0 or a.mutasi_out <> 0 or a.jual <> 0 or a.ret_jual <> 0 or jual_by_evap <> 0 or consin_by_evap <> 0) or 1 = 1) 
AND ((A.akhir <0) or 1 = 1)
