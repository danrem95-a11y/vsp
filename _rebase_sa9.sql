IF EXISTS(SELECT 1 FROM SYS.SYSPROCEDURE WHERE proc_name='sp_fa_rebase_jan_jun')
...
BEGIN
   ...
   INSERT INTO cred
   SELECT 'FA101'||CAST(YEAR(d.period) AS varchar(4))||RIGHT('0'||CAST(MONTH(d.period) AS varchar(2)),2),
          d.period, c.accum_dep_account, ROUND(SUM(d.depreciation_amount),0), c.category_name
   ...
   SELECT voucher,1,'101',tgl,'FA','412-066',SUM(amt),0,SUM(amt),0,'IDR',1,'N',voucher,'D',
          'Beban Penyusutan Aktiva Tetap '||CAST(MONTH(tgl) AS varchar(2))||'/'||CAST(YEAR(tgl) AS varchar(4))
   FROM cred GROUP BY voucher,tgl;

   INSERT INTO gl_journal (voucher,urut,site_id,tgl,modul_id,account_id,debet,kredit,debet_kurs,kredit_kurs,curr_id,rate_rp,posting,voucher_manual,dk,ket)
   SELECT cr.voucher, 1+(SELECT COUNT(*) FROM cred c2 WHERE c2.voucher=cr.voucher AND c2.acc<=cr.acc),
          '101',cr.tgl,'FA',cr.acc,0,cr.amt,0,cr.amt,'IDR',1,'N',cr.voucher,'K','Akum. Peny. '||cr.catname
   FROM cred cr;

   UPDATE FA_DEPRECIATION SET posting_status='P',
          journal_no='FA101'||CAST(YEAR(period) AS varchar(4))||RIGHT('0'||CAST(MONTH(period) AS varchar(2)),2)
   ...
   SELECT '101',tgl,'P','FA101'||CAST(YEAR(tgl) AS varchar(4))||RIGHT('0'||CAST(MONTH(tgl) AS varchar(2)),2),
