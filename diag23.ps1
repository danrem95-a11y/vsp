...
SELECT
    GG.voucher,
    MAX(GG.order_reff) AS ORDER_REFF,
    SUM(GG.kredit) AS K,
    SUM(GG.debet)  AS D,
...
WHERE GG.account_id = '226-001'
  ...
  AND EXISTS (
      SELECT 1 FROM SALDO_AWAL_FAKTUR S2
      WHERE S2.BUKTI_ID = GG.order_reff
        AND S2.TIPE_TRANS IN (1, 2)
        AND MONTH(S2.PERIODE) = 1
        AND YEAR(S2.PERIODE) = 2026)
GROUP BY GG.voucher
...
try {
    ...
    while ($r.Read()) {
        $net = [decimal]$r['NET']
        $sum += $net
        $out += "$($r['voucher'])|$($r['ORDER_REFF'])|$($r['K'])|$($r['D'])|$net"
    }
    $r.Close()
    $out += "TOTAL=$sum"
}
...
$out | Out-File 'c:\BTV\debug\diag23_out.txt' -Encoding UTF8