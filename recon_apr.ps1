...
function AprRecon($sinvper,$glend,$label){
$sql = @"
...
from (select gr.PERSEDIAAN acc, sum(s.nilai) sinv from sinv s join im_produk p on p.produk_id=s.stok_id join IM_PRODUCT_GROUP gr on gr.KODE_GROUP=p.group_product where s.periode='$sinvper' group by gr.PERSEDIAAN) sv
full outer join (select o.acc, o.opening+isnull(m.mv,0) gl from (select AccountCode acc, sum(AmountDebet-AmountCredit) opening from gl_balance where Period='2026-01-01' group by AccountCode) o left join (select account_id acc, sum(debet-kredit) mv from gl_journal where posting='P' and tgl between '2026-01-01' and '$glend' group by account_id) m on m.acc=o.acc where o.acc in (select distinct PERSEDIAAN from IM_PRODUCT_GROUP where isnull(PERSEDIAAN,'')<>'')) g on g.acc=sv.acc
where abs(isnull(sv.sinv,0)-isnull(g.gl,0)) > 100
order by selisih
"@
Write-Host ("=== "+$label+" : akun dgn selisih > 100 ===")
Reader $sql
}
AprRecon "2026-05-01" "2026-04-30" "APRIL (SINV 05/01 vs GL end-Apr) - KONDISI RUSAK"
Write-Host "`n=== TOTAL April ==="
Reader @"
...
from (select gr.PERSEDIAAN acc, sum(s.nilai) sinv from sinv s join im_produk p on p.produk_id=s.stok_id join IM_PRODUCT_GROUP gr on gr.KODE_GROUP=p.group_product where s.periode='2026-05-01' group by gr.PERSEDIAAN) sv
full outer join (select o.acc, o.opening+isnull(m.mv,0) gl from (select AccountCode acc, sum(AmountDebet-AmountCredit) opening from gl_balance where Period='2026-01-01' group by AccountCode) o left join (select account_id acc, sum(debet-kredit) mv from gl_journal where posting='P' and tgl between '2026-01-01' and '2026-04-30' group by account_id) m on m.acc=o.acc where o.acc in (select distinct PERSEDIAAN from IM_PRODUCT_GROUP where isnull(PERSEDIAAN,'')<>'')) g on g.acc=sv.acc
) x
"@
$cn.Close()
