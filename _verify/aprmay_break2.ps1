...
Qry "STOK Apr->Mei per akun: opening Mei vs (opening Apr + mutasi GL Apr)" @"
select g.acc,
  cast(isnull(oa.stk,0) as numeric(20,2))  opening_apr,
  cast(isnull(om.stk,0) as numeric(20,2))  opening_mei,
  cast(isnull(om.stk,0)-isnull(oa.stk,0) as numeric(20,2)) mutasi_stok,
  cast(isnull(mv.mov,0) as numeric(20,2))  mutasi_gl,
  cast((isnull(om.stk,0)-isnull(oa.stk,0))-isnull(mv.mov,0) as numeric(20,2)) selisih
from $acc g
...
# Item dgn qty=0 di kedua periode tapi masih ada nilai (residu) - atau nilai berubah tanpa qty
Qry "Ringkas: total selisih revaluasi-diam (qty sama, hpp beda) Apr->Mei + jumlah item" @"
select cast(sum(b.nilai-a.nilai) as numeric(20,2)) total_selisih_nilai, count(*) n_item
from sinv a join sinv b on b.stok_id=a.stok_id and b.periode='2026-05-01'
where a.periode='2026-04-01' and abs(a.qty-b.qty) < 0.001 and abs(a.hpp_avg-b.hpp_avg) > 0.0001
"@
$cn.Close()
