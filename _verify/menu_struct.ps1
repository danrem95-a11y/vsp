...
Qry "sysleftmenu kolom (header)" "select first * from sysleftmenu"
Qry "sysleftmenu baris FA/Aktiva (cari kolom yg memuat w_fa / aktiva)" "select * from sysleftmenu where lower(cast(sysleftmenu as varchar(2000))) like '%w_fa%' or 1=0"
Qry "sysgroupleftmenu kolom (header)" "select first * from sysgroupleftmenu"
$cn.Close()
