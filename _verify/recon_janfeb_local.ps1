...
function Val($sql){ $c=$cn.CreateCommand(); $c.CommandTimeout=600; $c.CommandText=$sql; try{$v=$c.ExecuteScalar(); if($v -eq $null -or $v -is [DBNull]){return 0}; return [decimal]$v}catch{return "ERR" } }
...
Write-Host ("{0,-4} | {1,-34} | {2,-34} | {3,-34}" -f 'Bln','AR opname-vs-Ledger(103-001)','AP opname-vs-Ledger(226)','STOK sinv-vs-Ledger(persed)')
Write-Host ("-"*120)
foreach($p in $periods){
  ...
  Write-Host ("{0,-4} | opn={1,13} led={2,13}  gap={3,12} | gap={4,15} | gap={5,15}" -f $lbl, (N $ar_opn),(N $ar_led),$arg,$apg,$stg)
}
$cn.Close()
