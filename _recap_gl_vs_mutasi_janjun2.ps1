...
function Tab([string]$q,[string]$label){
  ...
  try{ $m=$c.CreateCommand(); $m.CommandText=$q; $m.CommandTimeout=180; $rd=$m.ExecuteReader(); $fc=$rd.FieldCount
    ...
    $n=0; while($rd.Read()){ $n++; if($n -le 80){ $v=@(); for($i=0;$i -lt $fc;$i++){ $x=$rd.GetValue($i); if($x -is [decimal]-or $x -is [double]){$v+=('{0:N2}' -f $x)}else{$v+=[string]$x} }; Write-Output ("  "+($v -join ' | ')) } }
    if($n -gt 80){ Write-Output "  ... ($n baris)" }; if($n -eq 0){ Write-Output "  (0 baris)" }; $rd.Close()
  }catch{ Write-Output ("  ERR: "+$_.Exception.Message.Split([char]10)[0]) }
}

# mapping stok_id -> akun persediaan (materialize sekali)
$m=$c.CreateCommand()
$m.CommandText = "select count(*) from im_produk where isnull(group_product,'') <> ''"
Write-Output ("cek mapping tersedia: " + $m.ExecuteScalar())
...
select o.account_id,
 ...
 where abs(o.opname_sinv - isnull(g.ledger_gl,0)) > 1000
 order by abs(o.opname_sinv - isnull(g.ledger_gl,0)) desc
"@
  Tab $q "Selisih Opname vs GL -- akhir $($p.label) 2026 (hanya |selisih|>1000)"
}
$c.Close()
