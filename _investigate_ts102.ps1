...
select periode, cast(qty as numeric(12,2)) qty, cast(nilai as numeric(18,2)) nilai, cast(hpp_avg as numeric(18,2)) hpp_avg
  from sinv where stok_id='TS.102-1081' and periode between '2026-01-01' and '2026-07-31'
 order by periode
"@ "1. SINV TS.102-1081 2026"
...
select s1.tipe_trans, s1.tgl, s1.bukti_id, cast(s2.qty as numeric(12,2)) qty, cast(s2.netto as numeric(16,2)) netto, cast(isnull(s2.hpp,0) as numeric(16,2)) hpp
 ...
 where s1.bukti_id=s2.bukti_id and s2.stok_id='TS.102-1081'
   and s1.tgl between '2026-01-01' and '2026-06-30'
 order by s1.tgl
"@ "2. Penjualan/keluar TSALES2 TS.102-1081 Jan-Jun"
...
select t1.tipe_trans, t1.tgl, t1.bukti_id, cast(t2.qty as numeric(12,2)) qty, cast(t2.netto as numeric(16,2)) netto, cast(isnull(t2.hpp,0) as numeric(16,2)) hpp
 ...
 where t1.bukti_id=t2.bukti_id and t2.stok_id='TS.102-1081'
   and t1.tgl between '2026-01-01' and '2026-06-30'
 order by t1.tgl
"@ "3. Mutasi masuk TSTOK2 TS.102-1081 Jan-Jun"
$c.Close()
