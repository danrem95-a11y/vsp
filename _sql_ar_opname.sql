/* ============================================================================
   ...
   Fixes vs previous version:
     1. Removed MONTH(:arg_tgl1)=1 branch — ELSE already handles January correctly
            ...
            END)                                                                       AS NEW_RATE,
                ...
                ISNULL((SELECT SUM(GJ.debet)
                                ...
                                WHERE GJ.voucher = INV.ORDER_CLIENT
                                    ...
                                    AND GJ.debet > 0
                                    AND GJ.TGL >= :arg_tgl1
                                    AND GJ.TGL < DATEADD(day, 1, :arg_tgl2)), 0)                 AS MUTASI_IDR,
                ISNULL((SELECT SUM(CASE WHEN TP.FLAG_ORDER = 11 THEN ABS(TP.NILAI_BAYAR)
                                                                WHEN TP.FLAG_ORDER = 1 THEN -ABS(TP.NILAI_BAYAR)
                                                                ELSE 0 END)
                                FROM TBYR2_PUTIH TP
