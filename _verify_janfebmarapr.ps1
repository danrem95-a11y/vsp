...
Tab "select refresh_id,periode,ts_start,ts_end,status from refresh_ledger where periode in ('2026-01','2026-02','2026-03','2026-04') order by ts_start desc" "0. refresh_ledger Jan/Feb/Mar/Apr terbaru"
...
ValidasiChain "2026-01-01" "2026-01-31" "2026-02-01" "1. Saldo Akhir Jan (dihitung ulang) vs Saldo Awal Feb (tersimpan) -- target 0 baris"
ValidasiChain "2026-02-01" "2026-02-28" "2026-03-01" "2. Saldo Akhir Feb (dihitung ulang) vs Saldo Awal Mar (tersimpan) -- target 0 baris"
ValidasiChain "2026-03-01" "2026-03-31" "2026-04-01" "3. Saldo Akhir Mar (dihitung ulang) vs Saldo Awal Apr (tersimpan) -- target 0 baris"
...
select gr.persediaan account_id, count(*) n_voucher_mismatch, cast(sum(abs(v.mutasi-v.glval)) as numeric(20,2)) total_selisih
  from (
    select gr2.persediaan persediaan, s1.bukti_id,
     ...
     where s1.bukti_id=s2.bukti_id and s1.tipe_trans='88' and s1.tgl between '2026-01-01' and '2026-04-30'
...
"@ "4. WIP per-voucher independen (Jan-Apr): harus 0 baris"

Tab "select gate_id,periode_from,sumber,n_pelanggaran,status,tgl_cek from costing_gate_result order by tgl_cek desc" "5. costing_gate_result (masih kosong?)"
Tab "select result_id,tgl_check,periode_from,account_id,status from wip_out_gate_result order by tgl_check desc" "6. wip_out_gate_result (masih 3 baris lama?)"
$c.Close()
