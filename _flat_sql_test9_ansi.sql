...
isnull(mvmt.bln1_debit,0.00) as bln1_debit, isnull(mvmt.bln1_credit,0.00) as bln1_credit,
isnull(mvmt.bln2_debit,0.00) as bln2_debit, isnull(mvmt.bln2_credit,0.00) as bln2_credit,
isnull(mvmt.bln3_debit,0.00) as bln3_debit, isnull(mvmt.bln3_credit,0.00) as bln3_credit,
isnull(mvmt.bln4_debit,0.00) as bln4_debit, isnull(mvmt.bln4_credit,0.00) as bln4_credit,
isnull(mvmt.bln5_debit,0.00) as bln5_debit, isnull(mvmt.bln5_credit,0.00) as bln5_credit,
isnull(mvmt.bln6_debit,0.00) as bln6_debit, isnull(mvmt.bln6_credit,0.00) as bln6_credit,
isnull(mvmt.bln7_debit,0.00) as bln7_debit, isnull(mvmt.bln7_credit,0.00) as bln7_credit,
isnull(mvmt.bln8_debit,0.00) as bln8_debit, isnull(mvmt.bln8_credit,0.00) as bln8_credit,
isnull(mvmt.bln9_debit,0.00) as bln9_debit, isnull(mvmt.bln9_credit,0.00) as bln9_credit,
isnull(mvmt.bln10_debit,0.00) as bln10_debit, isnull(mvmt.bln10_credit,0.00) as bln10_credit,
isnull(mvmt.bln11_debit,0.00) as bln11_debit, isnull(mvmt.bln11_credit,0.00) as bln11_credit,
isnull(mvmt.bln12_debit,0.00) as bln12_debit, isnull(mvmt.bln12_credit,0.00) as bln12_credit,
a.fincatdes+' '+b.parentname+' '+c.accountcode as is_find
from gl_cate a
inner join gl_cate_detail b on a.fincatcode = b.fincatcode
inner join gl_acc c on b.parentcode = c.parentcode
left outer join (SELECT gl_balance.AccountCode, sum(AmountDebet) as debit, sum(AmountCredit) as credit FROM gl_balance WHERE gl_balance.Period = '2026-01-01' GROUP BY gl_balance.AccountCode) awal on c.accountcode = awal.accountcode
left outer join (
...
FROM gl_journal
WHERE tgl between '2026-01-01' and '2026-12-31' and ((isnull(gl_journal.show_hide,'1') = '1') or 1 = 0)
GROUP BY gl_journal.account_id
) mvmt on c.accountcode = mvmt.account_id
where (a.fincatcode = 'IS1001' or a.fincatcode = 'IS1110' or a.fincatcode = 'IS2010' or a.fincatcode = 'IS2110' or a.fincatcode = 'IS2120' or a.fincatcode = 'IS2130' or a.fincatcode = 'IS2230' or a.fincatcode = 'IS2330')
