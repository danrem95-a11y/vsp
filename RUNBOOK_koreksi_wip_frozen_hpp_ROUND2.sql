-- ============================================================================
-- RUNBOOK ROUND 2: Koreksi Cascade WIP-Out (TR.039A Mei/Juni, 16 baris)
-- Tanggal disiapkan : 2026-08-10 (setelah refresh April-Juni pasca Round 1)
--
-- KENAPA ADA ROUND 2?
-- --------------------
-- Round 1 (39 voucher) sudah dikoreksi & di-refresh April-Juni. Maret & April
-- sekarang STABIL (tidak muncul lagi di deteksi gap). TAPI: refresh April
-- ikut menghitung ULANG saldo rata-rata penutupan April (krn TSALES2 April yg
-- sudah kita koreksi di Round 1 ikut masuk formula penutupan) -- closing
-- April baru ini otomatis jadi saldo AWAL Mei. Akibatnya 8 voucher Mei (dan
-- turunannya 7 voucher Juni) yg SUDAH kita koreksi di Round 1 (memakai saldo
-- awal Mei versi LAMA) jadi stale LAGI relatif ke saldo awal Mei versi BARU.
--
-- Ini efek CASCADE yg melekat pada desain freeze immutable + moving-average:
-- koreksi WIP-Out di satu bulan mengubah rata-rata penutupan bulan itu, yg
-- adalah saldo awal bulan berikutnya. Bukan bug baru, bukan fix Round 1 yg
-- salah -- historis konvergen: Round 1 selisih Rp5-19jt/unit, Round 2 cuma
-- Rp362rb-1,33jt/unit (jauh lebih kecil, tanda sedang menuju stabil).
--
-- Mekanisme SAMA PERSIS dgn Round 1 (script ini reuse-able, deteksi otomatis
-- bukan hardcode voucher), cuma nama tabel staging diberi suffix _R2 supaya
-- tidak bentrok dgn tabel Round 1 (JANGAN dihapus, tetap sbg audit trail).
--
-- SYARAT: JANGAN refresh ulang lagi sebelum menjalankan script ini.
-- SETELAH script ini: refresh April-Juni SEKALI LAGI, lalu cek STEP 6a --
-- kemungkinan besar sudah 0 baris (konvergen), tapi verifikasi tetap wajib.
-- ============================================================================


-- ============================================================================
-- STEP 0: PETA + BACKUP (Round 2)
-- ============================================================================
SELECT z.stok_id, z.evap, z.wo_bukti, z.voucher, z.tgl_wo, z.qty,
  ...
  INTO ZZ_WIPFIX_MAP_R2_20260810
  FROM (
...
-- 0b. Verifikasi -- HARUS 16 baris (atau sesuai jumlah stale saat ini, cek dulu)
SELECT count(*) n_map,
       sum(case when bukti_id_22 is null then 1 else 0 end) n_22_null,
       cast(sum((hpp_new-hpp_old)*qty) as numeric(20,2)) total_delta
  FROM ZZ_WIPFIX_MAP_R2_20260810;
-- EXPECTED: n_map=16, n_22_null=0
-- JANGAN LANJUT jika n_22_null > 0 -- investigasi dulu.

-- 0c. Backup nilai LAMA (Round 2)
SELECT m.wo_bukti as bukti_id, s2.stok_id, s2.evap, s2.hpp as hpp_old
  INTO ZZ_BAK_WIPFIX_TSALES2_88_R2_20260810
  FROM ZZ_WIPFIX_MAP_R2_20260810 m, tsales2 s2
...
SELECT m.bukti_id_22 as bukti_id, s2.stok_id, s2.evap, s2.hpp as hpp_old
  INTO ZZ_BAK_WIPFIX_TSALES2_22_R2_20260810
  FROM ZZ_WIPFIX_MAP_R2_20260810 m, tsales2 s2
...
SELECT m.wipin_bukti as bukti_id, t2.stok_id,
  ...
  INTO ZZ_BAK_WIPFIX_TSTOK2_R2_20260810
  FROM ZZ_WIPFIX_MAP_R2_20260810 m, tstok2 t2
...
SELECT g.voucher, g.account_id, g.tgl, g.debet as debet_old, g.kredit as kredit_old, g.ket
  INTO ZZ_BAK_WIPFIX_GL_R2_20260810
  FROM gl_journal g
 WHERE g.voucher IN (SELECT DISTINCT voucher FROM ZZ_WIPFIX_MAP_R2_20260810)
   AND g.modul_id='AS' AND g.account_id IN ('102-020','102-001');

-- 0d. Verifikasi backup -- HARUS 16/16/16/32
SELECT (SELECT COUNT(*) FROM ZZ_BAK_WIPFIX_TSALES2_88_R2_20260810) n_bak_88,
       (SELECT COUNT(*) FROM ZZ_BAK_WIPFIX_TSALES2_22_R2_20260810) n_bak_22,
       (SELECT COUNT(*) FROM ZZ_BAK_WIPFIX_TSTOK2_R2_20260810)     n_bak_tstok2,
       (SELECT COUNT(*) FROM ZZ_BAK_WIPFIX_GL_R2_20260810)         n_bak_gl;
COMMIT;
...
UPDATE TSALES2
  ...
  FROM TSALES2, ZZ_WIPFIX_MAP_R2_20260810 m
...
-- ============================================================================
-- STEP 3: UPDATE TSALES2 -- baris '22'
-- ============================================================================
UPDATE TSALES2
   SET HPP = ROUND(m.hpp_new,2)
  FROM TSALES2, ZZ_WIPFIX_MAP_R2_20260810 m
...
UPDATE TSTOK2
  ...
  FROM TSTOK2, ZZ_WIPFIX_MAP_R2_20260810 m
...
-- EXPECTED: 0 (SETELAH refresh April-Juni sekali lagi pasca Round 2 ini)
...
SELECT voucher,
  ...
  FROM gl_journal
 WHERE voucher IN (SELECT DISTINCT voucher FROM ZZ_WIPFIX_MAP_R2_20260810)
   AND modul_id='AS'
 GROUP BY voucher
...
-- ROLLBACK (jika perlu)
-- ============================================================================
-- UPDATE TSALES2 SET HPP = b.hpp_old FROM TSALES2, ZZ_BAK_WIPFIX_TSALES2_88_R2_20260810 b
--  WHERE TSALES2.bukti_id=b.bukti_id AND TSALES2.stok_id=b.stok_id AND isnull(TSALES2.evap,'')=isnull(b.evap,'');
-- UPDATE TSALES2 SET HPP = b.hpp_old FROM TSALES2, ZZ_BAK_WIPFIX_TSALES2_22_R2_20260810 b
--  WHERE TSALES2.bukti_id=b.bukti_id AND TSALES2.stok_id=b.stok_id AND isnull(TSALES2.evap,'')=isnull(b.evap,'');
-- UPDATE TSTOK2 SET HPP=b.hpp_old, NETTO=b.netto_old, KOTOR=b.kotor_old, HRG=b.hrg_old, NETTO_HPP=b.netto_hpp_old
--  FROM TSTOK2, ZZ_BAK_WIPFIX_TSTOK2_R2_20260810 b WHERE TSTOK2.bukti_id=b.bukti_id AND TSTOK2.stok_id=b.stok_id;
-- UPDATE gl_journal SET debet=b.debet_old, kredit=b.kredit_old
--  FROM gl_journal, ZZ_BAK_WIPFIX_GL_R2_20260810 b
