...
def generate_one(n_months):
    ...
    header_lines = [
        ...
        'detail(height=' + str(detail_height) + ' color="536870912" transparency="0" gradient.color="8421504" gradient.transparency="0" gradient.angle="0" brushmode="0" gradient.repetition.mode="0" gradient.repetition.count="0" gradient.repetition.length="100" gradient.focus="0" gradient.scale="100" gradient.spread="100" )',
    ]
    ...
    group1_height = 100
    # Only ONE group level (by fincatcode) is declared -- exactly matching the proven reference
    # dw_rpt_is_flat_multibulan.srd (commit 90e3fb8), which uses sum(...for group 2) extensively
    # in its trailer.2 band and a trailer.2 band itself, WITHOUT ever declaring group(level=2) at
    # all, and got further through PowerBuilder's import than any version built in this session
    # that added an explicit group(level=2)/header.2 band. PB's DW syntax parser evidently
    # tolerates "for group 2" and a trailer.2 band with no declared level-2 group; adding one was
    # this session's own (wrong) theory and never actually validated.
    groups = [
