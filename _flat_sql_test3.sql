select
iq.fincatcode,iq.fincatdes,iq.parentcode,iq.parentname,iq.flag_dk,
sum(iq.awal_debit) as awal_debit,sum(iq.awal_credit) as awal_credit,
sum(iq.awal2_debit) as awal2_debit,sum(iq.awal2_credit) as awal2_credit,
sum(iq.bln1_debit) as bln1_debit,sum(iq.bln1_credit) as bln1_credit,
sum(iq.bln2_debit) as bln2_debit,sum(iq.bln2_credit) as bln2_credit,
sum(iq.bln3_debit) as bln3_debit,sum(iq.bln3_credit) as bln3_credit,
sum(iq.bln4_debit) as bln4_debit,sum(iq.bln4_credit) as bln4_credit,
sum(iq.bln5_debit) as bln5_debit,sum(iq.bln5_credit) as bln5_credit,
sum(iq.bln6_debit) as bln6_debit,sum(iq.bln6_credit) as bln6_credit,
sum(iq.bln7_debit) as bln7_debit,sum(iq.bln7_credit) as bln7_credit,
sum(iq.bln8_debit) as bln8_debit,sum(iq.bln8_credit) as bln8_credit,
sum(iq.bln9_debit) as bln9_debit,sum(iq.bln9_credit) as bln9_credit,
sum(iq.bln10_debit) as bln10_debit,sum(iq.bln10_credit) as bln10_credit,
sum(iq.bln11_debit) as bln11_debit,sum(iq.bln11_credit) as bln11_credit,
sum(iq.bln12_debit) as bln12_debit,sum(iq.bln12_credit) as bln12_credit,
iq.fincatdes+' '+iq.parentname as is_find
from
(
select
a.fincatcode,a.fincatdes,b.parentcode,b.parentname,
...
group by iq.fincatcode,iq.fincatdes,iq.parentcode,iq.parentname,iq.flag_dk
order by iq.fincatcode,iq.parentcode