...
"@
Tab "select stok_id,evap,voucher,tgl_wo,cast(hpp_old as numeric(16,2)) old,cast(hpp_new as numeric(16,2)) new_ from ($map) zz order by tgl_wo" "0b. Preview map NR (harus 3 baris, match tabel di atas)"
Tab "select count(*) n, sum(case when bukti_id_22 is null then 1 else 0 end) n22null, cast(sum((hpp_new-hpp_old)*qty) as numeric(20,2)) total from ($map) zz" "Ringkas (harus n=3, total=43.200.000,00)"
Tab @"
select count(*) n_tstok2_guard
  from ($map) zz, tstok2 t2x
 where t2x.bukti_id=('102'+substr(zz.bukti_id_22,4,11)) and t2x.stok_id=zz.stok_id and isnull(t2x.coa_id,'')=zz.evap
"@ "TSTOK2 guard match (harus 3, atau kurang jika belum ada WIP-in blm terjual)"
$c.Close()
