select fincatcode,parentcode,accountcode,max(fincatdes) as fincatdes,max(parentname) as parentname,max(flag_dk) as flag_dk,max(awal_debit) as awal_debit,max(awal_credit) as awal_credit,max(awal2_debit) as awal2_debit,max(awal2_credit) as awal2_credit,max(bln1_debit) as bln1_debit,max(bln1_credit) as bln1_credit,max(bln2_debit) as bln2_debit,max(bln2_credit) as bln2_credit,max(bln3_debit) as bln3_debit,max(bln3_credit) as bln3_credit,max(bln4_debit) as bln4_debit,max(bln4_credit) as bln4_credit,max(bln5_debit) as bln5_debit,max(bln5_credit) as bln5_credit,max(bln6_debit) as bln6_debit,max(bln6_credit) as bln6_credit,max(bln7_debit) as bln7_debit,max(bln7_credit) as bln7_credit,max(bln8_debit) as bln8_debit,max(bln8_credit) as bln8_credit,max(bln9_debit) as bln9_debit,max(bln9_credit) as bln9_credit,max(bln10_debit) as bln10_debit,max(bln10_credit) as bln10_credit,max(bln11_debit) as bln11_debit,max(bln11_credit) as bln11_credit,max(bln12_debit) as bln12_debit,max(bln12_credit) as bln12_credit from (select
...
) xwrap group by fincatcode,parentcode,accountcode