...
(
    ...
    (
        ...
        FROM (
            ...
            WHERE AT.TIPE_TRANS IN ('02', '05', '06', '12', '16')
              ...
              AND EXISTS (
                  SELECT 1 FROM gl_journal GG
                  WHERE GG.voucher    = AT.ORDER_CLIENT
                    AND GG.account_id = '226-001'
                    AND GG.kredit     > 0)

            UNION

            /* Faktur carry-over dari saldo awal Januari */
            SELECT SAF2.BUKTI_ID AS ORDER_CLIENT
            FROM SALDO_AWAL_FAKTUR SAF2
            WHERE SAF2.TIPE_TRANS IN (1, 2)
              AND MONTH(SAF2.PERIODE) = 1
              AND YEAR(SAF2.PERIODE)  = YEAR(:arg_tgl1)
              AND EXISTS (
                  SELECT 1 FROM gl_journal GG
                  WHERE GG.voucher    = SAF2.BUKTI_ID
                    AND GG.account_id = '226-001'
                    AND GG.kredit     > 0)
        ) JANGKAR
    ...
    LEFT JOIN (
        ...
        WHERE SAF_O.TIPE_TRANS IN (1, 2)
          ...
          AND EXISTS (
              SELECT 1 FROM gl_journal GG
              WHERE GG.voucher    = SAF_O.BUKTI_ID
                AND GG.account_id = '226-001'
                AND GG.kredit     > 0)
    ...
    LEFT JOIN (
        ...
        WHERE P.TGL >= DATEADD(day, 1 - DATEPART(dayofyear, :arg_tgl1), :arg_tgl1)
          ...
          AND EXISTS (
              SELECT 1 FROM gl_journal GG
              WHERE GG.voucher    = P.ORDER_CLIENT
                AND GG.account_id = '226-001'
                AND GG.kredit     > 0)
