    ...
    SELECT
        ...
        ISNULL((SELECT SUM(ISNULL(SAF_O.SALDO_KURS, 0))
                ...
                WHERE SAF_O.BUKTI_ID = INV.ORDER_CLIENT
                  AND SAF_O.TIPE_TRANS IN (1, 2)
                  AND SAF_O.PERIODE >= DATEADD(day, 1 - DATEPART(dayofyear, :arg_tgl1), :arg_tgl1)
                  AND SAF_O.PERIODE <  DATEADD(month, 1, DATEADD(day, 1 - DATEPART(dayofyear, :arg_tgl1), :arg_tgl1))), 0)
          + ISNULL((SELECT SUM(CASE WHEN P.TIPE_TRANS IN ('02', '05', '06', '16') THEN  P.TTL_NETTO
                    ...
                    WHERE P.ORDER_CLIENT = INV.ORDER_CLIENT
                      AND P.TGL >= DATEADD(day, 1 - DATEPART(dayofyear, :arg_tgl1), :arg_tgl1)
                      AND P.TGL  < :arg_tgl1
                      AND P.ORDER_OKE = 'Y'
          ...
          - ISNULL((SELECT SUM(ISNULL(T2.NILAI_BAYAR, 0))
                    ...
                    WHERE T2.BUKTI_ID = INV.ORDER_CLIENT
                      ...
                      AND T1.TGL  < :arg_tgl1), 0)                            AS SALDO_AWAL,
        ...
        ISNULL((SELECT SUM(CASE
          ...
          - ISNULL((SELECT SUM(ISNULL(T2.NILAI_BAYAR_IDR, 0))
                    ...
                    WHERE T2.BUKTI_ID = INV.ORDER_CLIENT
                      AND T1.FLAG_BAYAR IN (1, 2)
                      AND T1.TGL >= DATEADD(day, 1 - DATEPART(dayofyear, :arg_tgl1), :arg_tgl1)
                      AND T1.TGL  < :arg_tgl1), 0)                            AS SALDO_AWAL_IDR,
        ...
        ISNULL((SELECT SUM(CASE WHEN TP.FLAG_ORDER NOT IN (2, 22)
                ...
                WHERE TP.BUKTI_ID = INV.ORDER_CLIENT
                  ...
                  AND TP.TGL_BAYAR  < DATEADD(day, 1, :arg_tgl2)), 0)         AS ADJ,
        ISNULL((SELECT SUM(CASE WHEN TP.FLAG_ORDER NOT IN (2, 22)
                                     THEN  ABS(TP.NILAI_BAYAR_IDR)
                                ELSE     -ABS(TP.NILAI_BAYAR_IDR) END)
                FROM TBYR2_PUTIH TP
                WHERE TP.BUKTI_ID = INV.ORDER_CLIENT
                  AND TP.TGL_BAYAR >= :arg_tgl1
                  AND TP.TGL_BAYAR  < DATEADD(day, 1, :arg_tgl2)), 0)         AS ADJ_IDR,
        ...
        ISNULL((SELECT SUM(ISNULL(T2.NILAI_BAYAR, 0))
                ...
                WHERE T2.BUKTI_ID = INV.ORDER_CLIENT
                  AND T1.FLAG_BAYAR IN (1, 2)
                  AND T1.TGL >= :arg_tgl1
                  AND T1.TGL  < DATEADD(day, 1, :arg_tgl2)), 0)         AS NILAI_BAYAR,
        ISNULL((SELECT SUM(ISNULL(T2.NILAI_BAYAR_IDR, 0))
                FROM TBYR1 T1 INNER JOIN TBYR2 T2 ON T2.VOUCHER = T1.VOUCHER
                WHERE T2.BUKTI_ID = INV.ORDER_CLIENT
                  AND T1.FLAG_BAYAR IN (1, 2)
                  AND T1.TGL >= :arg_tgl1
                  AND T1.TGL  < DATEADD(day, 1, :arg_tgl2)), 0)         AS NILAI_BAYAR_IDR,
        (SELECT MAX(T1.VOUCHER_MANUAL)
         FROM TBYR1 T1 INNER JOIN TBYR2 T2 ON T2.VOUCHER = T1.VOUCHER
         WHERE T2.BUKTI_ID = INV.ORDER_CLIENT
           AND T1.FLAG_BAYAR IN (1, 2)
           AND T1.TGL >= :arg_tgl1
           AND T1.TGL  < DATEADD(day, 1, :arg_tgl2))                    AS VOUCHER_MANUAL,
